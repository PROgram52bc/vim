...
if [ -L "$HOME/.vimrc" ] && [ "$(readlink "$HOME/.vimrc")" -ef "$DIR/.vimrc" ]; then # if vimrc exists and is a link to the current directory
	#: # do nothing
...
elif [ -f "$HOME/.vimrc" ] || [ -L "$HOME/.vimrc" ]; then # if vimrc exists as a file or a link to some other file
	while true; do
	...
	while true; do
		...
		case $answer in
			y)
				...
				if curl -fLo "$PLUGPATH" --create-dirs https://raw.githubusercontent.com/junegunn/vim-plug/master/plug.vim; then
...
if [ -d "$DIR/UltiSnips" ]; then # if snippet folder exists in current directory
	if [ -L "$ULTISNIPSPATH" ] && [ "$(readlink "$ULTISNIPSPATH")" -ef "$DIR/UltiSnips" ]; then # if there is already a link pointing to the folder in current directory
		echo "$ULTISNIPSPATH already points to the current directory"
	elif [ -d "$ULTISNIPSPATH" ] || [ -L "$ULTISNIPSPATH" ]; then # if the folder exists, or is a link to other directory (or a broken link), ask for conflict resolution
	...
	case $answer in
		y)
			if [ ! -f "$HOME/.config/nvim/init.vim" ]; then
				...
				cat > "$HOME/.config/nvim/init.vim" << 'HERE'
...
HERE
			...
			for f in "$DIR"/nv/lua/*.lua; do
				dest="$HOME/.config/nvim/plugin/$(basename "$f")"
